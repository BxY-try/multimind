...
if (!API_KEY) {
  console.error("ALIBABA_API_KEY is not defined in environment variables")
}

const alibabaAI = new OpenAI({
  apiKey: API_KEY,
  baseURL: "https://dashscope-intl.aliyuncs.com/compatible-mode/v1",
})

/**
 * Format messages for Alibaba Qwen API (OpenAI compatible format)
 */
...
/**
 * Process image for Qwen API
 */
const processImageForQwen = (imageBase64, messages) => {
...
export const streamChatCompletion = async (modelId, messages, imageBase64 = null) => {
  try {
    ...
    if (imageBase64) {
      formattedMessages = processImageForQwen(imageBase64, formattedMessages)
    }

    const response = await alibabaAI.chat.completions.create({
      model: modelId,
    ...
    console.error("Alibaba Qwen service error:", error.message)
    throw error
  }
}
